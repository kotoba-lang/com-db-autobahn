(ns db_autobahn.main
  "Kotodama WASM entrypoint for the Db_autobahn clean-room actor (L4) — Clojure port.
  ...
  facts (`db_autobahn.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "db_autobahn")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"   :plural "accounts"   :id-prefix "dbautoba_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}           :refs {}}
   {:entity "Loan"      :plural "loans"      :id-prefix "dbautoba_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"db_autobahn.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "db_autobahn-compat" :tier tier :entities entities} 200])
...
(defn start! [] :db_autobahn-compat/ready)
